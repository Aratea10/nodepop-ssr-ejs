...
async function main() {
    ...
    const userKratos = await createSecureUser('kratos@example.com', 'Kratos', USER_PASSWORD['kratos@example.com']);
    const userZeus = await createSecureUser('zeus@example.com', 'Zeus', USER_PASSWORD['zeus@example.com']);
    const userAtenea = await createSecureUser('atenea@example.com', 'Atenea', USER_PASSWORD['atenea@example.com']);
    const userPoseidon = await createSecureUser('poseidon@example.com', 'Poseidón', USER_PASSWORD['poseidon@example.com']);
    const userHades = await createSecureUser('hades@example.com', 'Hades', USER_PASSWORD['hades@example.com']);
    const userAres = await createSecureUser('ares@example.com', 'Ares', USER_PASSWORD['ares@example.com']);
    const userHefesto = await createSecureUser('hefesto@example.com', 'Hefesto', USER_PASSWORD['hefesto@example.com']);
    const userHermes = await createSecureUser('hermes@example.com', 'Hermes', USER_PASSWORD['hermes@example.com']);
    const userHelios = await createSecureUser('helios@example.com', 'Helios', USER_PASSWORD['helios@example.com']);
    const userPersefone = await createSecureUser('persefone@example.com', 'Perséfone', USER_PASSWORD['persefone@example.com']);
    const userAtreus = await createSecureUser('atreus@example.com', 'Atreus', USER_PASSWORD['atreus@example.com']);
    const userFreya = await createSecureUser('freya@example.com', 'Freya', USER_PASSWORD['freya@example.com']);
    const userBaldur = await createSecureUser('baldur@example.com', 'Baldur', USER_PASSWORD['baldur@example.com']);
    const userMimir = await createSecureUser('mimir@example.com', 'Mímir', USER_PASSWORD['mimir@example.com']);
    const userThor = await createSecureUser('thor@example.com', 'Thor', USER_PASSWORD['thor@example.com']);
    const userOdin = await createSecureUser('odin@example.com', 'Odín', USER_PASSWORD['odin@example.com']);
    const userSindri = await createSecureUser('sindri@example.com', 'Sindri', USER_PASSWORD['sindri@example.com']);
    const userFaye = await createSecureUser('faye@example.com', 'Faye', USER_PASSWORD['faye@example.com']);
    const userAngrboda = await createSecureUser('angrboda@example.com', 'Angrboda', USER_PASSWORD['angrboda@example.com']);
    const userSurtr = await createSecureUser('surtr@example.com', 'Surtr', USER_PASSWORD['surtr@example.com']);

    const [
        kratos, zeus, atenea, poseidon, hades, ares, hefesto, hermes, helios,
        persefone, atreus, freya, baldur, mimir, thor, odin, sindri, faye, angrboda, surtr
    ] = await User.insertMany([
        userKratos, userZeus, userAtenea, userPoseidon, userHades, userAres,
        userHefesto, userHermes, userHelios, userPersefone, userAtreus, userFreya,
        userBaldur, userMimir, userThor, userOdin, userSindri, userFaye,
        userAngrboda, userSurtr
    ]);
    ...
    await Product.insertMany([
        ...
        { name: 'Cota de Malla de Baldur', owner: baldur._id, price: 2100, tags: ['lifestyle', 'work'] },
        ...
        { name: 'Cinturón de la Fuerza (Megingjörð)', owner: thor._id, price: 2300, tags: ['work', 'lifestyle'] },
